...
router.post('/create-invoice', async (req, res) => {
  try {
    const { products, supplier, shopId } = req.body;
    ...
    const newInvoice = new Invoice({ products, supplier, shop: shop._id });
    ...
    for (const productData of products) {
      ...
      await Product.findByIdAndUpdate(
        product,
        {
          $push: { invoices: { invoice: newInvoice._id, quantity, price } },
          $inc: { stock: quantity },
        }
      );
    }
...
router.get('/get-all-invoices', async (req, res) => {
  try {
    const invoices = await Invoice.find().populate('products.product').populate('shop');
...
router.get('/get-invoice/:id', async (req, res) => {
  try {
    const invoice = await Invoice.findById(req.params.id).populate('products.product').populate('shop');
    if (!invoice) {
